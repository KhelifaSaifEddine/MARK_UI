/*function month(startDay, maxDays, nameMonth, parent){
    ...
    parent.innerHTML += 
    `<h3 align="left" style ="background :black; margin :0px; width:80%;">`
		+ nameMonth + `2022 </h3>`
    ...
    // events
    $(document).on("click","." + nameMonth,function(){
        let temp = $(this).text()
        if(temp != ""){
...
month(7,31,"january",document.getElementById("calendar"))
...
*/
    ...
    // events
    $(parent).on("click","." + nameMonth,function(){
        let temp = $(this).text()
        if(temp != ""){
            $(this).toggleClass("checked");
            //$(this).html(temp + "<br/>X");
